...
class Logger
{
    ...
    static function saveRequestToSysLog($version) {
        ...
        $sql                        = "INSERT INTO `".LOGDB_NAME.
            "`.`api_sys_log` (`id`,`method`,`resource`,`params`,`date`,`response_code`,`user_ip`, `cpu`, `server`, `apiid`) VALUES ('".API_REQUEST_GUID."','".strval(METHOD)."','".$resource."','".addslashes(json_encode(DataParser::getArray()))."','".date("Y-m-d H:i:s",time())."','0','".UserHelper::GetUserIP()."', {{%logcpuload%}}, '{{%loghostname%}}', '".APIID."')";
